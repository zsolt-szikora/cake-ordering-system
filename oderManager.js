...
module.exports.fulfillOrder = (orderId, fulfillmentId) => {
  return getOrder(orderId).then(savedOrder => {
    const fulfilledOrder = createFulfilledOrder(savedOrder, fulfillmentId);
    console.info("CREATED fulfilledOrder\n"+ JSON.stringify(fulfilledOrder, null, 2));
    return saveOrder(fulfilledOrder).then(() => {
      console.info(`SAVED fulfilledOrder`);
      return placeOrderStream(fulfilledOrder);
    })
  })
}
